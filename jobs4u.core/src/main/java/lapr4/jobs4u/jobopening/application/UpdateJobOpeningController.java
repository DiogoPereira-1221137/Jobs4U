...
import lapr4.jobs4u.jobopening.domain.Mode;
...
//            jobOpeningCustomer.getId().toString().equals(customer.getId().toString())
    ...
    public boolean updateMode(JobOpening selected, Mode mode) {
        if(selected.updateMode(mode)) {
            jobOpeningRepository.save(selected);
            return true;
        }
        return false;
    }

    public boolean updateCType(JobOpening selected, String fullTime) {
        if(selected.updateCType(fullTime)) {
            jobOpeningRepository.save(selected);
            return true;
        }
        return false;
    ...
    public boolean updateAddress(JobOpening selected, Address address) {
        if(selected.updateAddress(address)) {
            jobOpeningRepository.save(selected);
            return true;
        }
        return false;
    ...
    public boolean updateDescription(JobOpening selected, String s) {
        if(selected.updateDescription(s)) {
            jobOpeningRepository.save(selected);
            return true;
        }
        return false;
    ...
    public boolean updateJF(JobOpening selected, String s) {
        if(selected.updateJF(s)) {
            jobOpeningRepository.save(selected);
            return true;
        }
        return false;
    ...
    public boolean updateVacancies(JobOpening selected, int numberOfVacancies) {
        if(selected.updateVacancies(numberOfVacancies)) {
            jobOpeningRepository.save(selected);
            return true;
        }
        return false;
    ...
    public boolean phaseCompatibility(JobOpening selected, int attribute) {
        String current = "null";
        if (selected.recruitmentProcess() != null) {
            current = String.valueOf(selected.currentPhase());
        }
        switch (current) {
            case "null":
                return true;
